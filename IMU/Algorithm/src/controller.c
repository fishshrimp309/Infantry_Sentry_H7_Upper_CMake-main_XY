...
static Controller_DeltaT_t Controller_GetDeltaT(uint32_t cnt_last)
{
    Controller_DeltaT_t delta_t;

    delta_t.cnt_last = cnt_last;
    delta_t.dt = DWT_GetDeltaT(&delta_t.cnt_last);
...
void Fuzzy_Rule_Implementation(FuzzyRule_t *fuzzyRule, float measure, float ref)
{
    ...
    delta_t = Controller_GetDeltaT(fuzzyRule->DWT_CNT);
    fuzzyRule->DWT_CNT = delta_t.cnt_last;
    fuzzyRule->dt = delta_t.dt;
...
float PID_Calculate(PID_t *pid, float measure, float ref)
{
    ...
    delta_t = Controller_GetDeltaT(pid->DWT_CNT);
    pid->DWT_CNT = delta_t.cnt_last;
    pid->dt = delta_t.dt;
...
{
    Controller_DeltaT_t delta_t = Controller_GetDeltaT(ffc->DWT_CNT);

    ffc->DWT_CNT = delta_t.cnt_last;
    ffc->dt = delta_t.dt;
...
{
    Controller_DeltaT_t delta_t = Controller_GetDeltaT(ldob->DWT_CNT);

    ldob->DWT_CNT = delta_t.cnt_last;
    ldob->dt = delta_t.dt;
...
float TD_Calculate(TD_t *td, float input)
{
    ...
    delta_t = Controller_GetDeltaT(td->DWT_CNT);
    td->DWT_CNT = delta_t.cnt_last;
    td->dt = delta_t.dt;
